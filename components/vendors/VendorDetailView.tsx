  ...
  return (
    <main className="flex-grow overflow-y-auto pb-40">
      <div className="px-4 pt-6 pb-3">
        ...
        <p className="text-base font-normal leading-normal pt-1 text-slate-600 dark:text-slate-400">
      ...
      <div className="px-4 pt-6">
        <h3 className="text-lg font-bold leading-tight tracking-[-0.015em] pb-3 text-slate-900 dark:text-white">
          Metrics
        </h3>
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1 rounded-xl bg-slate-100 dark:bg-white/5 p-4">
          ...
          {shows.length > 0 ? (
            shows.map((show) => (
              <Link
                key={show.id}
                href={`/show/${show.id}`}
                className="flex items-center gap-4 rounded-xl bg-slate-100 dark:bg-white/5 p-4 cursor-pointer hover:bg-slate-200 dark:hover:bg-white/10"
              >
          ...
          {safes.length > 0 ? (
            safes.map((safe) => (
              <Link
                ...
                className="flex items-center gap-4 rounded-xl bg-slate-100 dark:bg-white/5 p-4 cursor-pointer hover:bg-slate-200 dark:hover:bg-white/10"
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-slate-200 dark:bg-white/10">
                  <span className="material-symbols-outlined text-slate-600 dark:text-slate-300 text-3xl">
                    safety_check
                  </span>
                </div>
                <div className="flex-1">
